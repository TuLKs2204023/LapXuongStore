        ...
        <div class="wrapper bg-white">
            ...
            @foreach ($orderItems as $item)
                <div class="d-flex justify-content-start align-items-center list py-1">
                    ...
                    <div class="detail-price-tu">{{ number_format($item->product->fakePrice(), 0, ',', '.') . ' VND' }}
                    </div>
                </div>
            @endforeach
            <div class="table-responsive">
            ...
            <div class="row border rounded p-1 my-3">
                <div class="col-md-6 py-3">
                    <div class="d-flex flex-column align-items start"> <b>Billing Address</b>
                        ...
                        <p class="text-justify pt-2">{{ $order->user->address ?? 'No Address' }},
                            {{ $order->user->ward->name ?? 'No Ward' }},
                            {{ $order->user->district->name ?? 'No District' }},
                            {{ $order->user->city->name ?? 'No District' }}</p>
                    </div>
                </div>
                <div class="col-md-6 py-3">
                    <div class="d-flex flex-column align-items start"> <b>Shipping Address</b>
                        <p class="text-justify">{{ $order->name }} | {{ $order->phone }}</p>
                        <p class="text-justify pt-2">{{ $order->address }}, {{ $order->ward }}, {{ $order->district }},
                            {{ $order->city }}</p>
                    </div>
                </div>
            </div>
