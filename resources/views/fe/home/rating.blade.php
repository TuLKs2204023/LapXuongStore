<div class="co-item" data-index="{{ $rating->id }}">
    ...
    <div class="avatar-text">
        <div class="at-rating">
            @if ($rating->user->boughtProduct($rating->product_id))
        ...
        <div
            @if ($rating->user->boughtProduct($rating->product_id)) style="background-color: var(--violet-2nd) !important" class="badge rounded-pill bg-info text-light"
                @else
        ...
        @if (auth()->user())
            @if (auth()->user()->role == 'Admin' || auth()->user()->role == 'Manager')
                <a href="#" id="deletecomment" class="btn btn-outline-danger btn-sm">
