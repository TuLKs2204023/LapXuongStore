        ...
        @if (auth()->user()->role !== 'Customer')
            ...
            <div class="card">
                <div class="card-header">
                    <a class="btn btn-outline-primary my-btn-outline" href="{{ Route('admin.gpu.create') }}">
                ...
                <!-- /.card-header -->
                <div class="card-body">
                    <table id="gpusMgmt" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>Name</th>
...
@section('myJs')
    ...
    <script>
        document.addEventListener("DOMContentLoaded", (e) => {
            const cateTable = $("#gpusMgmt").DataTable({
                "responsive": true,
                "lengthChange": true,
                "autoWidth": true,
                "buttons": ["copy", "csv", "excel", "pdf", "print", "colvis"]
            });
            cateTable.buttons().container().appendTo('#gpusMgmt_wrapper .col-md-6:eq(0)');
            ...
            import('{{ asset('/js/KienJs/itemsDelete.js') }}').then((mCatesDelete) => {
                const catesDelete = mCatesDelete.ItemsDeleteHandler({
                    url: '{{ Route('admin.gpu.destroy') }}',
                    token: '{{ csrf_token() }}',
                    cateTable,
                    selectors: {
                        tableSelector: "#gpusMgmt tbody",
                    },
                });
            });
        });
