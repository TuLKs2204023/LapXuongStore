            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ Route($isUpdate ? 'admin.ramGroup.update' : 'admin.ramGroup.store') }}" method="post"
                        class="card-body myForm" enctype="multipart/form-data" id="createRamGroups">
                        @csrf
                        @if ($isUpdate)
                            @method('put')
                        ...
                        <fieldset class="exact-value row g-3">
                            ...
                            <div class="form-group row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" id="exact-value" name="value" class="form-control"
                                        rules="required|min:0,exclude" value="{{ $isUpdate ? $ramGroup->value : '' }}">
    ...
    <script type="module">
        ...
        document.addEventListener("readystatechange", (e) => {
            if (e.target.readyState === "complete") {
                // Input validation
                const productForm = new Validator('#createRamGroups');

                // Update select Ram options
                const ramGroupHandler = new GroupHandler({formSelector: "#createRamGroups"});
