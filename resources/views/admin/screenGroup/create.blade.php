            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ Route($isUpdate ? 'admin.screenGroup.update' : 'admin.screenGroup.store') }}"
                        method="post" class="card-body myForm" enctype="multipart/form-data" id="createScreenGroups">
                        @csrf
                        @if ($isUpdate)
                            @method('put')
                            ...
                            <div class="form-group row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" id="value" name="value" class="form-control"
                                        rules="required|min:0,exclude" value="{{ $isUpdate ? $screenGroup->value : '' }}">
                            ...
                            <div class="form-group row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" id="range-min" name="min" class="form-control range-value-min"
                                        rules="requiredOr:#range-max|min:0,exclude" value="{{ $isUpdate ? $screenGroup->min : '' }}">
                                    <span class="form-message heighter"></span>
                                </div>
    ...
    <script type="module">
        ...
        document.addEventListener("readystatechange", (e) => {
            if (e.target.readyState === "complete") {
                // Input validation
                const screenForm = new Validator('#createScreenGroups');

                // Update select Screen options
                const screenGroupHandler = new GroupHandler({formSelector: "#createScreenGroups"});
